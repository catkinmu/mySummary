...
var HTML_ = (function() {
    ...
    function parse_dom_table_addtitle(table, _opts, title) {
        var opts = _opts || {};
        var oss = opts.defaultCellStyle || {};  /*单元格样式  */
        if (DENSE != null) opts.dense = DENSE;
        var ws = opts.dense ? ([]) : ({});
        ...
        var range = { s: { r: 0, c: 0 }, e: { r: sheetRows+1 , c: 0 } };
        var merges = [], midx = 0;
        var R = 1, _C = 0, C = 0, RS = 0, CS = 0;
        var _title = title || ""; 
        var title_o = { t: 's', v: _title, s: oss };
        ...
        for (; R < sheetRows+1; ++R) {
            ...
            if (R == sheetRows)
            {
                merges.push({ s: { r: 0, c: 0 }, e: { r: 0, c: maxc-1 } });
            }
        }
        ws['!merges'] = merges;
        ws['!ref'] = encode_range(range);
...
function table_to_book(table, opts) {
    ...
    function table_to_book_addtitle(table, opts,title) {
        return sheet_to_workbook(parse_dom_table_addtitle(table, opts,title), opts);
    }
/* OpenDocument */
...
function writeSync(wb, opts) {
    ...
    if (o.type == "array")
    {
        o.type = "binary"; var out = (writeSync(wb, o)); o.type = "array"; return s2ab(out);
    }
...
var utils = {
    ...
    table_to_book_addtitle: table_to_book_addtitle,
